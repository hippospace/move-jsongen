...
pub struct Jsongen<'env> {
    ...
    env: &'env GlobalEnv,
    intermediate_output: &'env CompiledProgramWithIntermediateOutput,
        ...
        for module_env in self.env.get_modules() {
            if !module_env.is_script_module() {
                ...
                let filename = path
                    ...
                    .to_string_lossy()
                    .to_string();
                let mut address_filaname =
                    PathBuf::from(module_env.self_address().to_hex_literal());
                address_filaname.push(filename);
                self.output.push((
                    address_filaname.to_string_lossy().to_string(),
