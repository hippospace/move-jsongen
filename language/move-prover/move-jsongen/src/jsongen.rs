...
impl<'env> Jsongen<'env> {
    ...
    pub fn gen(mut self) -> Vec<(String, String)> {
        for module_env in self.env.get_modules() {
            if module_env.is_target() && !module_env.is_script_module() {
    ...
    fn gen_struct_fields(&self, indent: &str, struct_env: &StructEnv<'_>) {
        ...
        if num_fields == 0 {
            ...
            for (i, field) in struct_env.get_fields().enumerate() {
                let type_str = self.type_to_ts_type_str(&field.get_type(), &tctx, &type_params);
    ...
    fn type_to_ts_type_str(
        ...
        match ty {
            ...
            Type::Vector(type_box) => {
                ...
                let inner_ty_str = self.type_to_ts_type_str(inner_ty, tdc, type_params);
                format!("vector<{}>", inner_ty_str)
            }
            Type::Struct(module_id, struct_id, _inner_types) => {
                ...
                // FIXME: provide struct type params
                let type_param_str = "";
    ...
    fn gen_func_params(&self, indent: &str, func_env: &FunctionEnv<'_>) {
        ...
        if num_params == 0 {
            ...
            for (i, param) in func_env.get_parameters().iter().enumerate() {
                ...
                let type_str = self.type_to_ts_type_str(&param.1, &tctx, &type_params);
